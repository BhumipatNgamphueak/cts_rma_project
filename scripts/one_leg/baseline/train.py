"""
One-Leg Baseline training script.

Usage:
    python scripts/one_leg/baseline/train.py --num_envs 1024 --headless
"""
import argparse, sys
...
import cts_rma_project.tasks  # noqa — registers gym envs
from cts_rma_project.tasks.one_leg.baseline.one_leg_env_cfg    import OneLegEnvCfg
from cts_rma_project.tasks.one_leg.baseline.agents.rsl_rl_ppo_cfg import OneLegBaselinePPOCfg


def main():
    device = args_cli.device or "cuda"

    env_cfg = OneLegEnvCfg()
    env_cfg.scene.num_envs = args_cli.num_envs
    env_cfg.sim.device     = device

    env = gym.make("OneLeg-Baseline-v0", cfg=env_cfg)
    env = RslRlVecEnvWrapper(env)

    runner_cfg = OneLegBaselinePPOCfg()
    ...
    log_dir = os.path.join("logs", "one_leg", "baseline",
